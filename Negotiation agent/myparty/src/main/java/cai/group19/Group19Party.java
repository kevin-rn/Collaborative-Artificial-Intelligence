...
public class Group19Party extends DefaultParty {
	...
	private Map<String, ValueSetUtilities> utilities;

	private ArrayList<Bid> myBids;
	private ArrayList<Bid> opponentBids;
	private ArrayList<Offer> offers;
	private double nashmax;
	...
	public void notifyChange(Inform info) {
		try {
			if (info instanceof Settings) {
				...
				this.bwu = new BidsWithUtility((LinearAdditive) this.profileint.getProfile());

				this.space = (LinearAdditiveUtilitySpace) profileint.getProfile();
				this.utilities = space.getUtilities();
...
//				updateTargetUtility();
				...
				fop = fop.with(profileint.getProfile().getDomain(), null);
				opponentBids = new ArrayList<>();
				myBids = new ArrayList<>();
				offers = new ArrayList<>();
//				

//				LinearAdditiveUtilitySpace space =
...
//			    String jsonString = mapper.writeValueAsString(fin.getAgreement().getMap());
...
//				obj.put("opponentbids", this.opponentBids);
//				fw.write(obj.toString());
//				fw.close();

//				JSONObject obj = new JSONObject(jsonString);
//				System.out.println("obj: " + obj.get("geniusweb_exampleparties_conceder_Conceder_2"));
//				
...
//					i++;
	...
	private void makeOffer() throws IOException {
		Action action;
		if ((protocol.equals("SAOP")) && isGood(lastReceivedBid)) {
...
//			Bid bid = newBidAbsoluteTFT();
//			Bid bid = newBidAverageTFT();
//			Bid bid = newBidRelativeTFT();
			Bid bid = getTradeOffBid();
	...
	private boolean isGood(Bid bid) {
		...
		if (profile instanceof UtilitySpace) {
			if (profile.getReservationBid() != null) {
				double reservationBid = ((UtilitySpace) profile).getUtility(profile.getReservationBid()).doubleValue();

				if (targetUtility > reservationBid) {
					return ((UtilitySpace) profile).getUtility(bid).doubleValue() > targetUtility;
				} else {
					return ((UtilitySpace) profile).getUtility(bid).doubleValue() > reservationBid;
				}
			} else {
				return ((UtilitySpace) profile).getUtility(bid).doubleValue() > targetUtility;
			}
...
//				+ "progress.get(T) = " + progress.get(T));
		...
		// TODO: check whether to also check for getTerminationTime()
		if (progress.get(T) == 1L) {
			return true;
		} else {
			return false;
		}
...
//				(((UtilitySpace) profile).getUtility(this.lastReceivedBid).doubleValue() > targetUtility));
	...
	private Bid getParetoOptimalBid() throws IOException {
		...
		ImmutableList<Bid> bids = bwu.getBids(new Interval(targetutil, maxutil));
		if (bids.size().compareTo(BigInteger.ZERO) == 0)
			return null;
	...
	/**
	 * Calculates the bid for the new Offer using the Trade-Off Strategy.
	 * 
	 * @return Bid for the offer to the other party.
	 */
	private Bid getTradeOffBid() {
		// Search for bids which have a similar utility (or 0.05 less) than the
		// opponents offer.
		Profile profile;
		try {
			profile = profileint.getProfile();
		...
		// Check if this party is the first to send a bid to the other party.
		if (this.lastSentBid == null || this.lastReceivedBid == null) {
			return bwu.getExtremeBid(true);
		}

		BigDecimal utilityLastBid = ((UtilitySpace) profile).getUtility(lastSentBid).add(new BigDecimal("0.005"));
		BigDecimal minConcession = new BigDecimal("0.05");
		BigDecimal maxConcession = BigDecimal.ZERO;

		// Search for bids which have a similar utility (or 0.05 less) than the
		// opponents offer.
		ImmutableList<Bid> bids = bwu.getBids(new Interval(utilityLastBid.subtract(minConcession), utilityLastBid));

		// Handles the case when no bids are found except the lastSentBid.
		while (bids.size().compareTo(BigInteger.ONE) <= 0) {
			maxConcession = minConcession;
			minConcession = minConcession.add(new BigDecimal("0.05"));
			bids = bwu.getBids(
					new Interval(utilityLastBid.subtract(minConcession), utilityLastBid.subtract(maxConcession)));

			if (minConcession.doubleValue() >= utilityLastBid.doubleValue()) {
				return null;
			}
		...
		Bid mostPreferable = lastSentBid;

		// Loop through bids and calculate similarity scores.
		for (Bid current : bids) {
			if (current.equals(lastSentBid) || !isGood(lastSentBid)) {
				continue;
			}
			...
			for (Entry<String, Value> issue : current.getIssueValues().entrySet()) {
				double weight = this.space.getWeight(issue.getKey()).doubleValue();
				currentValue = this.utilities.get(issue.getKey()).getUtility(issue.getValue()).doubleValue();
				receivedValue = this.utilities.get(issue.getKey()).getUtility(lastReceivedBid.getValue(issue.getKey()))
						.doubleValue();

				similarity += weight * (1 - Math.abs(currentValue - receivedValue));
			}
			if (similarity > highestSimilarity) {
				highestSimilarity = similarity;
				mostPreferable = current;
			}
		}
...
//			return null;
	...
	private Bid newBidRelativeTFT() throws IOException {
		...
		BigDecimal last = ((UtilitySpace) profile).getUtility(this.lastReceivedBid);
		BigDecimal secondLast = ((UtilitySpace) profile).getUtility(this.secondLastReceivedBid);
		// BigDecimal diff = secondLast.subtract(last); //How much the newly done bid is
		// better for yourself
		BigDecimal change = last.divide(secondLast, RoundingMode.HALF_UP);
		BigDecimal newUtility = this.lastMadeBidUtility.multiply(change).min(BigDecimal.ONE); // New utility changes
																								// with -diff.
		BidsWithUtility bwu = new BidsWithUtility((LinearAdditive) this.profileint.getProfile());
		ImmutableList<Bid> bids = bwu.getBids(new Interval(newUtility.subtract(new BigDecimal("0.05")), newUtility));
	...
	private Bid getNewBid() throws IOException {
		...
		ImmutableList<Bid> bids = bwu.getBids(new Interval(targetutil, range.getMax()));
...
//						System.out.println(max + " < " + newutilproduct.doubleValue());
					...
					System.out.println("nashmax: " + fop.getUtility(maxBid).doubleValue() * targetUtility
							+ " with targetutil: " + this.targetUtility);
				}
			}
	...
	private double getUtilityOther(ArrayList<Offer> offers, String otherDomain, boolean agreement) {
		try {
			...
			Object obj = parser.parse(new FileReader("src/test/resources/"
					+ this.profileint.getProfile().getDomain().getName() + "/" + otherDomain + ".json"));
...
//				System.out.println("issue weight: " + issueWeights.get(issue));
					JSONObject values = (JSONObject) ((JSONObject) (((JSONObject) issueUtilities.get(issue))
							.get("discreteutils"))).get("valueUtilities");
//				System.out.println("issue utility: " + values.get(bid.getValue(issue).toString().replace("\"", "")));
					...
					// System.out.println(StringUtils.difference("1 TB",
					// (bid.getValue(issue).toString())));
